...
@section('content')
  ...
  @if( $heroEnabled )
    ...
    <div class="container-fluid g-0 m-0 flex-x mb-4 center-x center-y" style="border-bottom: 2px solid #ffc107;">
      ...
      @if(count($heroItems) > 1)
        ...
        <div class="container-fluid g-0 m-0 flex-x center-x center-y" style="position: absolute; top: 46vh;">
          @foreach($heroItems as $item)
            <button type="button" class="btn btn-warning hero-button mx-2 @if($loop->index == 0) highlighted-hero-button @endif" style="width: 50px;" data-index="{{ $loop->index }}"></button>
          @endforeach
        </div>
      @endif
